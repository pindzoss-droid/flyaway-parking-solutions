import { createContext, useCallback, useContext, useEffect, useState, type ReactNode } from "react";

type Lang = "bs" | "en";

const dict = {
  bs: {
    "nav.why": "Zašto Park&Fly",
    "nav.location": "Lokacija",
    "nav.faq": "FAQ",
    "nav.contact": "Kontakt",
    "nav.book": "Rezerviši online",
    "nav.admin": "Admin",
    "hero.title": "PARK&FLY",
    "hero.subtitle": "privatni parking nadomak aerodroma Sarajevo",
    "hero.desc":
      "Sigurno mjesto za vaše vozilo, samo 1 minut od aerodroma. 24/7 videonadzor, fizičko obezbjeđenje, besplatan transport do aerodroma i opcija rent a car kad god vam zatreba.",
    "hero.cta": "REZERVIŠI ONLINE",
    "hero.badge": "1 min od aerodroma · 24/7 nadzor",
    "why.title": "Zašto Park&Fly",
    "why.subtitle": "Sve što vam treba za miran odlazak na put.",
    "why.cctv.t": "Videonadzor 24/7",
    "why.cctv.d": "Kompletan parking je pokriven kamerama uz fizičko obezbjeđenje cijelo vrijeme.",
    "why.airport.t": "Blizina aerodroma",
    "why.airport.d": "Samo 1 minuta vožnje do terminala aerodroma Sarajevo.",
    "why.shuttle.t": "Besplatan transport",
    "why.shuttle.d": "Vozimo vas do aerodroma i čekamo pri povratku — bez dodatnih troškova.",
    "why.rentcar.t": "Rent a car",
    "why.rentcar.d": "Trebate auto na destinaciji ili po povratku? Imamo provjerenu rent a car ponudu.",
    "drone.title": "Pogled iz vazduha",
    "drone.desc": "Tražite siguran parking u blizini aerodroma Sarajevo? Naš parking je udaljen samo 350 metara — najbliži aerodromu. Zahvaljujući toj blizini, gotovo je nemoguće zakasniti na let, čak i kad su gužve i neočekivane situacije u gradu. Nudimo sigurno i povoljno mjesto za vaše vozilo u ograđenom prostoru sa video nadzorom 24/7.",
    "drone.p1": "Samo 350 m od aerodroma — najbliži parking",
    "drone.p2": "Sigurno i povoljno parkiranje",
    "drone.p3": "Ograđen prostor sa video nadzorom 24/7",
    "faq.title": "Često postavljana pitanja",
    "faq.q1": "Koliko je parking udaljen od aerodroma?",
    "faq.a1": "Parking je oko 1 minuta vožnje od terminala aerodroma Sarajevo. Naš shuttle vozi goste besplatno.",
    "faq.q2": "Da li je transport do aerodroma uračunat u cijenu?",
    "faq.a2": "Da, transport do aerodroma i nazad uračunat je u cijenu parkinga.",
    "faq.q3": "Šta ako mi let kasni ili se vraćam ranije?",
    "faq.a3": "Nema problema — javite nam i prilagodićemo vrijeme dolaska i odlaska.",
    "faq.q4": "Mogu li otkazati rezervaciju?",
    "faq.a4": "Da, rezervaciju možete otkazati pozivom ili porukom najkasnije 12 sati prije dolaska.",
    "location.title": "Lokacija",
    "location.desc": "Pronađite nas u blizini aerodroma Sarajevo. Tačna lokacija na mapi.",
    "video.title": "Kako doći do nas",
    "video.desc": "Pogledajte kratak vodič kako stići do našeg parkinga.",
    "reviews.title": "Šta kažu naši gosti",
    "reviews.desc": "Provjereni utisci na Google recenzijama.",
    "footer.rights": "Sva prava zadržana.",
    "footer.tagline": "Parkiraj. Leti. Mirno.",
    "rentcar.title": "RENT A CAR — provjerena ponuda",
    "rentcar.desc": "Rezervišite vozilo uz svoj parking po posebnoj cijeni.",
    "rentcar.cta": "Saznaj više",
    // form
    "form.title": "Online rezervacija parkinga",
    "form.fullname": "Ime i prezime",
    "form.plate": "Registracija vozila",
    "form.email": "Kontakt email",
    "form.phone": "Kontakt telefon",
    "form.range": "Trajanje zakupa",
    "form.arrival": "Vrijeme dolaska",
    "form.departure": "Vrijeme odlaska",
    "form.arrivalDate": "Datum dolaska",
    "form.departureDate": "Datum odlaska",
    "form.arrivalTime": "Vrijeme dolaska",
    "form.departureTime": "Vrijeme odlaska",
    "form.destination": "Destinacija leta",
    "form.transfer": "Potreban prevoz do aerodroma",
    "form.note": "Napomena",
    "form.estimate": "Procjena cijene",
    "form.submit": "Potvrdi rezervaciju",
    "form.checking": "Provjeravam dostupnost…",
    "form.available": "Termin je dostupan",
    "form.unavailable": "Termin nije dostupan — pokušajte drugi datum",
    "form.blocked": "Parking ne radi u odabranom periodu",
    "form.success": "Rezervacija poslana! Potvrda stiže na email.",
    "form.error": "Greška pri slanju rezervacije.",
    "form.pickdates": "Odaberite datume",
    "form.days": "dana",
    "form.rate": "Tarifa",
    "form.perDay": "KM/dan",
    "form.savings": "Štedite {x} KM zahvaljujući popustu za duži boravak!",
    // rent a car banner
    "rentcar.bannerTitle": "Rent a Car i usluge prevoza",
    "rentcar.bannerDesc": "Rezerviši vozilo ili transfer u 2 jednostavna koraka",
    // location labels
    "location.address": "Adresa",
    "location.phone": "Telefon",
    // footer
    "footer.ctaDesc": "Rezerviši online — sigurno mjesto za tvoj auto dok ti letiš.",
    "footer.call": "Pozovi",
    "footer.trust1": "24/7 nadzor",
    "footer.trust2": "Otvoreno 0–24h",
    "footer.trust3": "Kartice & gotovina",
    "footer.trust4": "1 min do aerodroma",
    "footer.about": "Privatni parking 1 min od aerodroma Sarajevo. Sigurno, brzo, povoljno.",
    "footer.hours": "Radno vrijeme",
    "footer.daysRange": "Pon – Ned",
    "footer.openAll": "0 – 24h",
    "footer.holidays": "Praznici",
    "footer.open": "Otvoreno",
    "footer.links": "Linkovi",
    "footer.contact": "Kontakt",
    // pricing
    "pricing.eyebrow": "Cjenovnik",
    "pricing.title": "Tri paketa, jedna sigurnost",
    "pricing.subtitle": "Što duže ostavite auto kod nas — to je dnevna tarifa povoljnija. Bez skrivenih troškova, sa transferom u cijeni.",
    "pricing.popular": "Najpopularnije",
    "pricing.savings": "Ušteda",
    "pricing.perDay": "/ dan",
    "pricing.book": "REZERVIŠI",
    "pricing.daysRange1": "1 – {to} dana",
    "pricing.daysRange2": "{from} – {to} dana",
    "pricing.daysRange3": "{from}+ dana",
    "pricing.p1.eyebrow": "Kratki boravak",
    "pricing.p1.title": "Brzi odlazak",
    "pricing.p1.tagline": "Savršeno za vikend putovanja i poslovne skokove — ostavi auto i kreni bez razmišljanja.",
    "pricing.p2.eyebrow": "Najbolji izbor",
    "pricing.p2.title": "Odmor bez brige",
    "pricing.p2.tagline": "Idealan balans cijene i mira — pokrije godišnji odmor uz uštedu na svaki dan.",
    "pricing.p3.eyebrow": "Dugi put",
    "pricing.p3.title": "Maraton paket",
    "pricing.p3.tagline": "Za one koji putuju dugo — najniža dnevna tarifa i potpuna sigurnost vašeg vozila.",
    "pricing.f1": "Besplatan transfer do aerodroma",
    "pricing.f2": "Video nadzor 24/7",
    "pricing.f3": "Fizički nadzor",
    "pricing.f4": "Ograđen i osvijetljen prostor",
  },

  en: {
    "nav.why": "Why Park&Fly",
    "nav.location": "Location",
    "nav.faq": "FAQ",
    "nav.contact": "Contact",
    "nav.book": "Book online",
    "nav.admin": "Admin",
    "hero.title": "PARK&FLY",
    "hero.subtitle": "private parking next to Sarajevo Airport",
    "hero.desc":
      "A safe spot for your vehicle, just 1 minute from the airport. 24/7 CCTV, on-site security, free airport shuttle and rent-a-car on demand.",
    "hero.cta": "BOOK ONLINE",
    "hero.badge": "1 min from airport · 24/7 monitoring",
    "why.title": "Why Park&Fly",
    "why.subtitle": "Everything you need for a worry-free trip.",
    "why.cctv.t": "24/7 CCTV",
    "why.cctv.d": "Full camera coverage with on-site security at all times.",
    "why.airport.t": "Next to the airport",
    "why.airport.d": "Just a 1-minute drive to Sarajevo Airport terminal.",
    "why.shuttle.t": "Free shuttle",
    "why.shuttle.d": "We drive you to the terminal and pick you up on return — at no extra cost.",
    "why.rentcar.t": "Rent a car",
    "why.rentcar.d": "Need a car at your destination or upon return? Vetted rental offer included.",
    "drone.title": "Aerial view",
    "drone.desc": "Looking for safe parking near Sarajevo Airport? Our parking is just 350 metres away — the closest to the airport. That proximity makes it nearly impossible to miss your flight, even during traffic and unexpected situations. We offer a safe, affordable spot for your vehicle in a fenced area with 24/7 video surveillance.",
    "drone.p1": "Only 350 m from the airport — closest parking",
    "drone.p2": "Safe and affordable parking",
    "drone.p3": "Fenced area with 24/7 video surveillance",
    "faq.title": "Frequently asked questions",
    "faq.q1": "How far is the parking from the airport?",
    "faq.a1": "About a 1-minute drive. Our shuttle is free for all guests.",
    "faq.q2": "Is the airport transfer included?",
    "faq.a2": "Yes, transfer to and from the airport is included in the parking price.",
    "faq.q3": "What if my flight is delayed or I return early?",
    "faq.a3": "No problem — just let us know and we will adjust pickup time.",
    "faq.q4": "Can I cancel a reservation?",
    "faq.a4": "Yes, you can cancel by phone or message at least 12 hours before arrival.",
    "location.title": "Location",
    "location.desc": "Find us right next to Sarajevo Airport. Exact location on the map.",
    "video.title": "How to find us",
    "video.desc": "A short video guide to our parking.",
    "reviews.title": "What our guests say",
    "reviews.desc": "Verified Google reviews.",
    "footer.rights": "All rights reserved.",
    "footer.tagline": "Park. Fly. Relax.",
    "rentcar.title": "RENT A CAR — vetted offer",
    "rentcar.desc": "Book a car with your parking at a special rate.",
    "rentcar.cta": "Learn more",
    "form.title": "Online parking reservation",
    "form.fullname": "Full name",
    "form.plate": "Vehicle plate",
    "form.email": "Contact email",
    "form.phone": "Contact phone",
    "form.range": "Booking range",
    "form.arrival": "Arrival time",
    "form.departure": "Departure time",
    "form.arrivalDate": "Arrival date",
    "form.departureDate": "Departure date",
    "form.arrivalTime": "Arrival time",
    "form.departureTime": "Departure time",
    "form.destination": "Flight destination",
    "form.transfer": "Airport transfer needed",
    "form.note": "Note",
    "form.estimate": "Estimated price",
    "form.submit": "Confirm reservation",
    "form.checking": "Checking availability…",
    "form.available": "Slot available",
    "form.unavailable": "Slot unavailable — please pick another date",
    "form.blocked": "Parking is closed for the selected period",
    "form.success": "Reservation sent! Confirmation is on its way to your email.",
    "form.error": "Failed to submit the reservation.",
    "form.pickdates": "Pick dates",
    "form.days": "days",
    "form.rate": "Rate",
    "form.perDay": "KM/day",
    "form.savings": "You save {x} KM thanks to the long-stay discount!",
    "rentcar.bannerTitle": "Rent a Car and transfer services",
    "rentcar.bannerDesc": "Book a vehicle or transfer in 2 simple steps",
    "location.address": "Address",
    "location.phone": "Phone",
    "footer.ctaDesc": "Book online — a safe spot for your car while you fly.",
    "footer.call": "Call",
    "footer.trust1": "24/7 monitoring",
    "footer.trust2": "Open 24/7",
    "footer.trust3": "Cards & cash",
    "footer.trust4": "1 min to airport",
    "footer.about": "Private parking 1 min from Sarajevo Airport. Safe, fast, affordable.",
    "footer.hours": "Opening hours",
    "footer.daysRange": "Mon – Sun",
    "footer.openAll": "0 – 24h",
    "footer.holidays": "Holidays",
    "footer.open": "Open",
    "footer.links": "Links",
    "footer.contact": "Contact",
    "pricing.eyebrow": "Pricing",
    "pricing.title": "Three packages, one peace of mind",
    "pricing.subtitle": "The longer you leave your car with us — the lower the daily rate. No hidden fees, transfer included.",
    "pricing.popular": "Most popular",
    "pricing.savings": "Save",
    "pricing.perDay": "/ day",
    "pricing.book": "BOOK NOW",
    "pricing.daysRange1": "1 – {to} days",
    "pricing.daysRange2": "{from} – {to} days",
    "pricing.daysRange3": "{from}+ days",
    "pricing.p1.eyebrow": "Short stay",
    "pricing.p1.title": "Quick getaway",
    "pricing.p1.tagline": "Perfect for weekend trips and business hops — drop your car and go.",
    "pricing.p2.eyebrow": "Best value",
    "pricing.p2.title": "Worry-free holiday",
    "pricing.p2.tagline": "The ideal balance of price and peace of mind — covers your vacation with daily savings.",
    "pricing.p3.eyebrow": "Long trip",
    "pricing.p3.title": "Marathon package",
    "pricing.p3.tagline": "For long travels — the lowest daily rate and full security for your vehicle.",
    "pricing.f1": "Free airport transfer",
    "pricing.f2": "24/7 video surveillance",
    "pricing.f3": "On-site security",
    "pricing.f4": "Fenced and lit area",
  },

} as const;

type Key = keyof typeof dict["bs"];

type Ctx = { lang: Lang; setLang: (l: Lang) => void; t: (k: Key) => string };

const I18nCtx = createContext<Ctx | null>(null);

export function I18nProvider({ children }: { children: ReactNode }) {
  const [lang, setLangState] = useState<Lang>("bs");
  useEffect(() => {
    const saved = typeof window !== "undefined" ? (localStorage.getItem("pf_lang") as Lang | null) : null;
    if (saved === "bs" || saved === "en") setLangState(saved);
  }, []);
  const setLang = useCallback((l: Lang) => {
    setLangState(l);
    if (typeof window !== "undefined") localStorage.setItem("pf_lang", l);
  }, []);
  const t = useCallback((k: Key) => dict[lang][k] ?? k, [lang]);
  return <I18nCtx.Provider value={{ lang, setLang, t }}>{children}</I18nCtx.Provider>;
}

export function useI18n() {
  const ctx = useContext(I18nCtx);
  if (!ctx) throw new Error("useI18n must be used within I18nProvider");
  return ctx;
}
